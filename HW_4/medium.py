...
def integrate(f, a, b, n_iter=1000):
    start_log = (time.time(), f"starting integrate function {f.__name__} from {a} to {b}")
    acc = 0
    step = (b - a) / n_iter
    for i in range(n_iter):
        acc += f(a + i * step) * step
    finish_log = (time.time(), f"finished integrate function {f.__name__} from {a} to {b}")
    return ((start_log, finish_log), acc)


def integrate_threads(f, a, b, *, n_jobs=1, n_iter=1000):
    logs.append((time.time(), f"integrating with threads with n_jobs={n_jobs}"))
    res = []
    with ThreadPoolExecutor(max_workers=n_jobs) as executor:
        step = (b - a) / n_jobs
        for i in range(n_jobs):
            res.append(executor.submit(integrate, f, a + i * step, a + (i + 1) * step, n_iter // n_jobs))
